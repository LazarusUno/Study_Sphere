...
function saveUserData() {
    ...
    let userData = {
      ...
      gender: $("#gender").val(),
      dob: String(new Date($("#dob").val()).getDate()).padStart(2, '0') + "/" + String(new Date($("#dob").val()).getMonth() + 1).padStart(2, '0') + "/" + new Date($("#dob").val()).getFullYear(),
      email: $("#email").val(),
      number: $("#number").val(),
      address: $("#address").val() + "\n" + $("#zip").val() + "\n" + ($("#city").val() == null ? "" : $("#city").val() + ", ") + $("#state").val() + ", " + $("#country").val(),
      website: $("#website").val().trim(),
      linkedIn: $("#linkedIn").val().trim(),
  
      // Education (loop through each accordion item)
      education: [],
      work: [],
      skills: [],
      interests: [],
      ...
      // Achievements
      achievements: $("#achv_description").val().replaceAll("\n", "<br />\r\n"),
  
      // Profile
      profile: $("#profile").val().replaceAll("\n", "<br />\r\n"),
    };
  
    // Loop through education sections and populate the education array
    for (let i = 0; i < $('#accordionEdu .accordion-item').length; i++) {
      ...
      if (degree != "" && school != "" && !isNaN(srt_date) && !isNaN(end_date)) {
        userData.education.push({ degree, srt_date, end_date, school });
      }
    }
  
    // Similar loops for work, skills, interests, and languages
  
    // Convert user data object to a JSON string
    let jsonData = JSON.stringify(userData, null, 2); // 2 for indentation (optional)
  ...
  // Call the saveUserData function when needed (e.g., on a button click)
  $(document).on('click', '#saveUserDataButton', function() {
    saveUserData();
  });
  

  document.getElementById("printCv1").addEventListener("click", function() {
    window.location.href = "/dash.html"; // Replace "dashboard.html" with your actual dashboard page
  });
